...
class LayerDelayRNN(nn.Module):
    def __init__(
        ...
        self.delay_gate = nn.Sequential(
            nn.Linear(input_size + hidden_size, self.max_delay)
        ).to(self.device)
    ...
    def forward(self, x, lengths=None, out_lengths=None, train = False):
        ...
        for t in range(max_seq_len):
            ...
            delay_gate_output = self.delay_gate(cell_input)
            one_hot_delay_gate = F.gumbel_softmax(delay_gate_output, hard=True) 
            delay_outs.append(one_hot_delay_gate.argmax(dim=-1).float().mean().item())
            
            self.DelayBuffer.buffer_update(x_t, one_hot_delay_gate)
        ...
        logs["delay_outs_mean"] = sum(delay_outs) / len(delay_outs)
        logs["delay_outs_var"] = sum((d - logs["delay_outs_mean"]) ** 2 for d in delay_outs) / len(delay_outs)
