...
class LearnableDelayRNN(nn.Module):
    def __init__(self, batch_size:int, input_size:int, hidden_size:int, output_size:int, max_delay:int, config:Config):
        super().__init__()
    ...
    def forward(self,
        ...
        finished_mask = torch.zeros(x.size(0), dtype=torch.bool, device=self.device)
        # 각 샘플의 실제 길이를 저장 (기본값은 Max Steps)
        actual_lengths = torch.full((x.size(0),), steps, dtype=torch.long, device=self.device)
        ...
        for t in range(steps):
            ...
            # --- [핵심 로직] 출력층의 마지막 뉴런 감시 ---
            # y_t shape: [Batch, Output_Size]
            # y_t[:, -1]: 마지막 뉴런의 값들
            current_trigger = y_t[:, -1] > 1.0
            
            # 이번 스텝에서 처음으로 1을 넘은 샘플들 찾기
            # (이미 끝난 샘플은 다시 갱신하지 않음)
            newly_finished = current_trigger & (~finished_mask)
            
            if newly_finished.any():
                # 종료된 시점을 기록 (t + 1은 현재 스텝까지 포함한다는 의미)
                actual_lengths[newly_finished] = t + 1
                finished_mask = finished_mask | newly_finished
            
            # 배치 내 모든 샘플이 종료 조건을 만족했으면 Loop 탈출
            if finished_mask.all():
                break
        
        # Loop가 일찍 끝났으면, 뒤쪽의 0으로 남은 부분 잘라내고 반환
        return y[:, :max_t_reached, :], actual_lengths